...
@Component({
  selector: 'kt-administrationandsections',
  templateUrl: './administrationandsections.component.html',
  styleUrls: ['./administrationandsections.component.scss']
})
export class AdministrationandsectionsComponent implements OnInit {
  ...
  AVAILABLE_HIGHERLEVEL: string[];
  AVAILABLE_JOBTITTLE: string[];
  constructor(
  ...
  ngOnInit() {
    this.AVAILABLE_TYPE=['ONE','TWO','THREEE','FOUR','FIVE','SIX'];
	this.AVAILABLE_HIGHERLEVEL=['ONE','TWO','THREEE','FOUR','FIVE','SIX'];
	this.AVAILABLE_JOBTITTLE=['doctor','engineer','artist','singer','player','nurse'];
    ...
    type: [this.product.model,Validators.required],
	higherlevel: [this.product.model,Validators.required],
	codefirstjobindepartment: [this.product.model],
	jobtittle: [this.product.model],
	requirednumber: [this.product.model],
		});
  ...
  goBack(id) {
	...
	getComponentTitle() {
		let result = 'new Administration And Sector';
